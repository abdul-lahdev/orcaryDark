...
// Dummy Data for Different Users
const INITIAL_CHATS = [
...
export default function ChatPage() {
  ...
  const [activeChat, setActiveChat] = useState(INITIAL_CHATS[1]); // Liam selected by default
  const [input, setInput] = useState("");

  // Separate message history for each chat
  ...
  // Auto-scroll fix for Radix UI ScrollArea
  useEffect(() => {
  ...
  const handleSendMessage = (e) => {
    ...
    setChats(prev => prev.map(c =>
      c.id === activeChat.id ? { ...c, lastMsg: input, time: "Now", unread: 0 } : c
    ));
      ...
      <div className="w-[384px] flex flex-col bg-[#2B2B31CC] rounded-[10px]">
        <div className="p-5 space-y-4">
          <div className="flex justify-between items-center text-white">
            ...
            <button className="p-1 hover:bg-white/5 rounded-lg transition-colors cursor-pointer"><Plus size={24} className="text-(--grey1)" /></button>
          </div>
          ...
          <div className="space-y-1 pb-4">
            {chats.map((chat) => (
              <div
                ...
                className={`p-4 rounded-[8px] w-[99%] mx-auto flex gap-3 cursor-pointer transition-all duration-200 ${activeChat.id === chat.id ? 'bg-white text-black shadow-lg scale-[1.02]' : 'hover:bg-white/5 text-white'
                  }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    <h4 className={`font-medium text-[16px]/[24px] truncate ${activeChat.id === chat.id ? 'text-(--dark3)' : 'text-white'}`}>{chat.name}</h4>
                    {chat.unread > 0 && activeChat.id !== chat.id && (
                      <div className="absolute -top-1 right-3 translate-y-10 -translate-x-5 size-5 bg-(--blue1) rounded-full text-white text-[12px] font-medium flex items-center justify-center" >10</div>
                    )}
                    <span className={`text-[12px] font-normal ${activeChat.id === chat.id ? 'text-(--dark3)' : 'text-(--dark6)'}`}>{chat.time}</span>
                  </div>

                  <p className={`text-[12px] truncate ${activeChat.id === chat.id ? 'text-(--dark3) font-medium' : 'text-(--grey1)'}`}>{chat.lastMsg}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 flex flex-col bg-[#1E1E22] rounded-[24px] border border-white/5 shadow-xl overflow-hidden">
        ...
        <ScrollArea ref={scrollRef} className="flex-1 p-6">
          <div className="space-y-6">
            {(chatHistories[activeChat.id] || []).map((msg, idx) => (
              <React.Fragment key={msg.id}>
                {/* Optional: Add Date Separator if it's a new day */}
                {idx === 2 && (
                ...
                <div className={`flex flex-col ${msg.isMe ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`max-w-[75%] p-4 rounded-2xl text-[14px] leading-relaxed shadow-sm transition-all ${msg.isMe
