          ...
          item.category === "Cardiology" && (
            ...
            >
              <div className="relative aspect-video rounded-[12px] overflow-hidden">
                <Image
                  src={item.thumbnail || "/images/classRoom/thumbnail.jpg"}
                  alt="Video Thumbnail"
                  width={376}
                  height={227}
    ...
    <>
      ...
      >
        <div className="w-full max-w-100 mx-auto">
          {" "}
          {/* Card ki width control karne ke liye */}
          <div className="relative aspect-3/4 w-full overflow-hidden rounded-[8px] border border-white/5">
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
              <Button
                variant="secondary"
                className="bg-white/10 backdrop-blur-md border-white/20 text-white hover:bg-(--blue1) hover:border-transparent transition-all"
              >
                View Document
              </Button>
            </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="bg-[#2B2B31] border-white/10 text-white sm:max-w-125 md:max-w-[35%] p-5 rounded-[20px]">
          <DialogHeader>
            <DialogTitle className="text-[18px] font-semibold text-(--grey7)">
              {!insufficientBal ? "Access Resource" : "Insufficient Balance"}
            </DialogTitle>
          </DialogHeader>

          {!insufficientBal && (
            <div className="flex items-center gap-6 py-3 text-start w-full md:w-[80%] mx-auto">
              <Image
                ...
                alt=""
              />
              <p className="text-[20px] font-normal text-(--grey1)">
                This is a paid resource you will need $14 to access. This amount
                will be credited from your wallet
              </p>
            </div>
          )}
          {insufficientBal && (
            <div className="flex flex-col items-center justify-center gap-6 py-3 text-center w-full md:w-[80%] mx-auto">
              <Image
                src="/images/resource/insufficientBalance.png"
                width={175}
                height={173}
                alt=""
              />
              <p className="text-[20px] font-normal text-(--grey1)">
                Looks like you don’t have sufficient amount to join the access
                resource.
              </p>
            </div>
          )}
